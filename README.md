...
1. Drop a bank CSV export, then run `uv run finances import-csv path/to/export.csv --out transactions/example-checking.csv --account "Example Checking" --opening 3000.00 --closing 2875.50`
2. Set `MERCURY_API_TOKEN` in your environment. Never commit it. Then `uv run finances pull --account-id <id> --opening 3000.00 --out transactions/example-checking.csv`
